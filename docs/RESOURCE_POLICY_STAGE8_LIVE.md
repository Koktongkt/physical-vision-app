...
Request admission consumes the ASGI body stream under the 4,000,000-byte global bound before parsing. Multipart parser scratch stays in memory with a spool threshold equal to that already-enforced bound and is explicitly closed after extraction or parser failure. Consequently, live preview uploads do not use Starlette's default 1 MiB temporary-disk rollover and do not leave multipart scratch open after the response.
